...
def smape(y_true, y_pred, mask=False):
    ...
    error = control_flow_ops.cond(mask, lambda: check_infinity(diff/total),
...
def quantile_loss(y_true, y_pred, quantiles):
    ...
    quantile_loss = (quantiles * clip_ops.clip_by_value(diff, 0., np.inf) +
                    (1 - quantiles) * clip_ops.clip_by_value(-diff, 0., np.inf))

    M = y_true.shape[0]
    error = quantile_loss / M
    sum_quantiles = math_ops.reduce_sum(error, axis=-1)

    return math_ops.reduce_sum(sum_quantiles,
                               axis=math_ops.range(sum_quantiles.shape.rank-1))
